<template>
	<div v-if="typeof classroom_lectures.data != 'undefined'">	
		<div class="card card-custom shadow-none border gutter-b" v-for="lecture in classroom_lectures.data" :key="lecture.id">
			<div class="card-body p-4">
				<div class="d-flex align-items-center">
					...
					<div class="d-flex flex-column flex-grow-1">
						<router-link :to="{ name: 'lecture.view', params: { id: lecture.lecture.id }}" class="text-dark-75 text-hover-primary mb-1 font-size-lg font-weight-bold">{{ lecture.lecture.title }}</router-link>
						<div class="d-flex">
							<div class="d-flex align-items-center pr-5">
								<span class="text-muted">{{ lecture.created_at }}</span>
							</div>
						</div>
					</div>
				...
				<b-collapse :id="'lecture_'+lecture.id">
				<div class="pt-3">
		...
		<div class="mt-7" v-if="classroom_lectures.data.length == 0 ">
			<div class="">
				<p class="text-muted">Tidak ada materi yang dibagikan</p>
			</div>
		</div>
		<b-pagination
